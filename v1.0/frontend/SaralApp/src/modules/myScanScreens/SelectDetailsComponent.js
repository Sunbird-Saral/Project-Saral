  ...
  onDropDownSelect = (index, value, type) => {
    ...
    if (type == 'class') {
      if (value != selectedClass) {
        ...
        let sectionListArr = [];
        _.forEach(sections, sectionData =>
          sectionListArr.push(sectionData.section),
        );
        this.setState(
          {
      ...
      let setData = [];
      let hasSetData =
        this.state.set.length > 0
          ? this.state.set[Number(index)]
            ? this.state.set[Number(index)].length >= 0
              ? -1
              : 0
            : 0
          : 0;
  ...
  async componentDidUpdate(prevProps) {
    if (prevProps != this.props) {
      ...
      if (calledStudentsData) {
        if (
          ...
          this.setState(
            ...
            async () => {
              if (
                studentsAndExamData.status &&
                studentsAndExamData.status == 200
              ) {
                if (
                  studentsAndExamData.data.students &&
                  studentsAndExamData.data.students.length > 0
                ) {
                  let obj = {
                    class: selectedClass,
                    classId: selectedClassId,
                  ...
                  if (studentsExamDataSaved) {
                    ...
                    _.filter(studentsAndExamData.data.exams, function (o) {
                      ...
                      set.push(
                        o.hasOwnProperty('set') && o.set.length > 0
                          ? o.set
                          : [],
                      );
                    });
            ...
            async () => {
              if (loginData.status && loginData.status == 200) {
                ...
                if (loginSaved) {
                  ...
                  Alert.alert(
                    Strings.message_text,
                    Strings.process_failed_try_again,
                    [
                      {
                        text: Strings.cancel_text,
                        style: Strings.cancel_text,
                        onPress: () => loader(false),
                      },
                      {
                        text: Strings.retry_text,
      ...
      if (calledScanStaus) {
        if (
          getScanStatusData &&
          prevProps.getScanStatusData != getScanStatusData
        ) {
      ...
      if (calledAbsentStatus) {
        if (
          absentStudentDataResponse &&
          prevProps.absentStudentDataResponse != absentStudentDataResponse
        ) {
          this.setState({calledAbsentStatus: false, callApi: ''});
          if (
            absentStudentDataResponse.status &&
            absentStudentDataResponse.status == 200
          ) {
            ...
            if (
              ...
              if (getStudentExamCache != null) {
                let result = getStudentExamCache.findIndex(e =>
                  ...
                  e.section == selectedSection &&
                  e.hasOwnProperty('subject')
                    ? e.subject == this.state.selectedSubject
                    : false,
                );
                let hasSubject = getStudentExamCache.findIndex(
                  e =>
                ...
                let resultDataIndex = getStudentExamCache.findIndex(
                  e =>
                    e.key == loginData.data.school.schoolId &&
                    e.class == selectedClass &&
                    e.section == selectedSection,
                );
                let hasSetValue = getStudentExamCache.findIndex(e =>
                  e.key == loginData.data.school.schoolId &&
                  e.class == selectedClass &&
                  e.section == selectedSection &&
                  e.hasOwnProperty('subject') &&
                  e.hasOwnProperty('set')
                    ? e.set == setValue
                    : false,
                );
                ...
                if (resultDataIndex > -1 && subjBool == false) {
                  getStudentExamCache[resultDataIndex].subject =
                    this.state.selectedSubject;
                  getStudentExamCache[resultDataIndex].data =
                    studentsAndExamData;
                } else if (result > -1) {
                  getStudentExamCache[result].data = studentsAndExamData;
                } else {
        ...
        <ShareComponent
          ...
          message={
            this.state.logmessage
              ? JSON.stringify(this.state.logmessage, null, 2)
              : ''
          }
            ...
            <View
              ...
              <View
                ...
                <DropDownMenu
                  options={classList}
                  onSelect={(idx, value) =>
                    this.onDropDownSelect(idx, value, 'class')
                  }
              ...
              {classListIndex != -1 && (
                <View
                  style={[
                    ...
                    {
                      paddingBottom:
                        sectionListIndex != -1 && sectionValid ? 0 : '10%',
                    },
                  ]}>
                  <View style={{flexDirection: 'row'}}>
                    <Text style={[styles.labelTextStyle]}>
                  ...
                  <DropDownMenu
                    options={sectionList}
                    onSelect={(idx, value) =>
                      this.onDropDownSelect(idx, value, 'section')
                    }
                    defaultData={defaultSelected}
              ...
              {sectionListIndex != -1 && sectionValid && (
                <View
                  ...
                  <DropDownMenu
                    options={subArr}
                    onSelect={(idx, value) =>
                      this.onDropDownSelect(idx, value, 'sub')
                    }
                    defaultData={defaultSelected}
              ...
              {ExamSetArray &&
                ExamSetArray.length > 0 &&
                ExamSetArray[subIndex] != null && (
                  <View
                    style={[
                      styles.fieldContainerStyle,
                      {
                    ...
                    <DropDownMenu
                      ...
                      onSelect={(idx, value) =>
                        this.onDropDownSelect(idx, value, 'set')
                      }
                      defaultData={
                        ExamSetArray && ExamSetArray[subIndex] == ''
                          ? ['NONE']
                          : defaultSelected
                      }
