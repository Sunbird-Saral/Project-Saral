...
class HomeComponent extends Component {
  ...
  saveDataInDB = async () => {
    ...
    const autoSyncBatchSize =
      Object.keys(loginData).length > 0 &&
      loginData.data.school.hasOwnProperty('autoSyncBatchSize')
        ? loginData.data.school.autoSyncBatchSize
        : 10;
    ...
    if (data != null) {
      ...
      if (len >= autoSyncBatchSize) {
        ...
        data.map(element => {
          ...
          var self = this;
          axios
            .put(apiObj.apiEndPoint(), apiObj.getBody(), {
              headers: apiObj.getHeaders(),
              cancelToken: source.token,
            })
            .then(function (res) {
              let localDataResponse = self.removeItemFromLocalStorage(
                res,
                data,
              );
              if (localDataResponse.length == 0) {
                setScannedDataIntoLocal(localDataResponse);
                self.hitPushNotification(
                  'Uploaded',
                  Strings.auto_sync_completed,
                );
              }
              apiResponse = res;
              ...
              if (
                typeof apiObj.getNextStep === 'function' &&
                res.data &&
            ...
            .catch(function (err) {
              ...
              let data = {
                ...
                message:
                  'Something went wrong with background process, please contact Admin',
                isOkAvailable: false,
                isCancel: false,
              };
  ...
  async componentDidMount() {
    ...
    const bgTimer =
      Object.keys(loginData).length > 0 &&
      loginData.data.school.hasOwnProperty('autoSyncFrequency')
        ? loginData.data.school.autoSyncFrequency
        : 600000;
    setInterval(async () => {
      const hasAutoSync =
        Object.keys(loginData).length > 0 &&
        loginData.data.school.hasOwnProperty('autoSync') &&
        loginData.data.school.autoSync
          ? true
          : false;
  ...
  async componentDidUpdate(prevProps) {
    ...
    if (multiBranding && prevProps.multiBranding != multiBranding) {
      if (multiBranding.status && multiBranding.status == 200) {
        ...
        const isMinimalModedata =
          this.props.loginData &&
          this.props.loginData.data &&
          this.props.loginData.data.school &&
          this.props.loginData.data.school.hasOwnProperty('isMinimalMode')
            ? this.props.loginData.data.school.isMinimalMode
            : null;
    ...
    if (
      studentsAndExamData &&
      prevProps.studentsAndExamData != studentsAndExamData
    ) {
  ...
  async callMultiBrandingActiondata() {
    ...
    if (hasCacheData && cacheFilterData.length > 0) {
      storeFactory.dispatch(
        this.dispatchBrandingDataApi(cacheFilterData[0].data),
      );
    } else if (hasNetwork) {
  ...
  render() {
    ...
    const BrandLabel =
      ...
      this.props.multiBrandingData.screenLabels &&
      this.props.multiBrandingData.screenLabels.homeScreen &&
      this.props.multiBrandingData.screenLabels.homeScreen[0];

    if (
      this.props.multiBrandingData === undefined ||
      this.props.multiBrandingData === null ||
    ...
    return (
      <View
        style={{
          flex: 1,
          backgroundColor: this.props.multiBrandingData.themeColor2
            ? this.props.multiBrandingData.themeColor2
            : AppTheme.WHITE,
        ...
        <View>
          <TouchableOpacity
            ...
            {BrandLabel && BrandLabel.assessmentLogo ? (
              <Image
                style={{height: 80, width: 80, resizeMode: 'stretch'}}
                source={{
                  uri:
                    BrandLabel &&
                    'data:image/png;base64,' + BrandLabel.assessmentLogo,
                }}
              />
            ) : (
              <Image
                style={{height: 80, width: 60, resizeMode: 'stretch'}}
                source={Assets.assessments}
              />
            )}
          </TouchableOpacity>
          <Text style={{textAlign: 'center', fontSize: 18, fontWeight: 'bold'}}>
            {BrandLabel && BrandLabel.useCase1
              ? BrandLabel.useCase1
              : 'Assessments'}
          </Text>
          ...
          {loginData && loginData.useCase3 === true && (
            <View style={{marginTop: 10}}>
              <TouchableOpacity
                ...
                {BrandLabel && BrandLabel.assessmentLogo ? (
                  <Image
                    style={{height: 100, width: 80, resizeMode: 'stretch'}}
                    source={{
                      uri:
                        BrandLabel &&
                        'data:image/png;base64,' + BrandLabel.assessmentLogo,
              ...
              <Text
                style={{textAlign: 'center', fontSize: 18, fontWeight: 'bold'}}>
                {BrandLabel && BrandLabel.useCase3
                  ? BrandLabel.useCase3
                  : 'Use Case 3'}
              </Text>
            </View>
          ...
          {loginData && loginData.useCase4 === true && (
            <View style={{marginTop: 10}}>
              ...
              <Text
                style={{textAlign: 'center', fontSize: 18, fontWeight: 'bold'}}>
                {BrandLabel && BrandLabel.useCase4
                  ? BrandLabel.useCase4
                  : 'Use Case 4'}
              </Text>
            </View>
          ...
          {loginData && loginData.useCase5 === true && (
            <View style={{marginTop: 10}}>
              ...
              <Text
                style={{textAlign: 'center', fontSize: 18, fontWeight: 'bold'}}>
                {BrandLabel && BrandLabel.useCase5
                  ? BrandLabel.useCase5
                  : 'Use Case 5'}
              </Text>
            </View>
