...
class ScanDetailsComponent extends Component {
    ...
    componentDidMount() {
        ...
        navigation.addListener('willFocus', payload => {
            ...
            if (ocrLocalResponse && ocrLocalResponse.response) {
                ...
                } else if(data.scannerType == SCAN_TYPES.SAT_TYPE) {
    ...
    renderSatData = () => {
        const { studentsScanData, satRollNo } = this.state
            ...
            <View style={{ flex: 1, backgroundColor: AppTheme.WHITE_OPACITY }}>
                ...
                {studentClass.length > 0 && section.length > 0 &&
                    <View>
                        ...
                        {scanTypeData.scanType == SCAN_TYPES.PAT_TYPE && <Text style={styles.tabLabelStyle}>{`${Strings.exam_sub_date}: ${selectedSubject} - ${examDate}`}</Text>}
                    </View>
                }
                {!summary ? <View>
                    <ScrollView
                    ...
                    >
                        {studentsScanData && studentsScanData.length > 0 &&
                            <View style={styles.container1}>
                                {scanTypeData.scanType == SCAN_TYPES.PAT_TYPE ? this.renderPatStudnetsData() : this.renderSatData()}
...
export default (connect(mapStateToProps, mapDispatchToProps)(ScanDetailsComponent));