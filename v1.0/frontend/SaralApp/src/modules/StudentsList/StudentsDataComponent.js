...
const StudentsDataComponent = ({
  ...
  const setStdMarkAsPrsAbst = async (data, chkPresent) => {
    ...
    if (stdData != null) {
      ...
      if (filterStdData.length > 0) {
        isStudentScannedInLocal = filterStdData[0].studentsMarkInfo.filter(
          o => {
    ...
    const isSheetScanned =
      typeof scanedData === 'object' && scanedData.hasOwnProperty('data')
        ? scanedData.data.length > 0 &&
          scanedData.data.filter(o => {
            // if (hasSet != null && hasSet.length >= 0) {
            //     return o.studentId == data.studentId && o.studentAvailability === true && o.marksInfo.length > 0 && hasSet == o.set
            // } else
            {
    ...
    if (isStudentPresent) {
      ...
      if (
        ...
        isSheetScanned.length == 0 &&
        isStudentScannedInLocal.length == 0
      ) {
        saveStudentIntoLocalStorage(
          data.studentId,
          isStudentPresent,
          filterStdData,
          stdData,
  ...
  const saveStudentIntoLocalStorage = async (
    studentId,
    studentAvailability,
    filterStdData,
    localStdData,
    ...
    if (filterStdData.length > 0) {
      let chkStdPresent = filterStdData[0].studentsMarkInfo.findIndex(
        val => val.studentId == studentId && val.marksInfo.length == 0,
      );
      if (chkStdPresent > -1) {
      ...
      let findIndex = localStdData.findIndex(el => {
        let findSection = el.studentsMarkInfo.some(
          item => item.section == filteredData.section,
        );
        if (
          el.classId === filteredData.class &&
          el.subject === filteredData.subject &&
          findSection &&
          el.examDate == filteredData.examDate &&
          el.examId == filteredData.examTestID
        ) {
          return true;
        }
      });
      localStdData[findIndex].studentsMarkInfo =
        filterStdData[0].studentsMarkInfo;
        ...
        <View style={{width: '80%', backgroundColor: AppTheme.WHITE}}>
          <View
            style={{
              borderWidth: 1,
              borderColor: '#DADADA',
              height: item.name.length > 50 ? 40 : 25,
              justifyContent: 'center',
            }}>
