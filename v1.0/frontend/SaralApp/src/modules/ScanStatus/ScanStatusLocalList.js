  ...
  const onScanClick = async () => {
    ...
    if (!hasUpdate) {
      SystemSetting.getBrightness().then(brightness => {
        //setState({oldBrightness: brightness});
      });

      let permissionStatus = await askPermissions();

      if (permissionStatus == 'granted') {
        let hasEmpty = roiData.hasOwnProperty('config')
          ? true
          : roiData.length > 0;
        if (minimalFlag && roiIndex != -1) {
          if (!hasEmpty) {
            ...
            openCameraActivity();
            onScanButtonClickEvent(loginData.data.school.schoolId);
          }
        } else if (!minimalFlag) {
          if (
            ...
            loginData.data.school.hasOwnProperty('offlineMode') == false ||
            (loginData.data.school.offlineMode == false && hasEmpty)
          ) {
            openCameraActivity();
            onScanButtonClickEvent(loginData.data.school.schoolId);
          } else {
      ...
      <View
        ...
        <View
          style={{
            width: '10%',
            height:
              studentName[0] && studentName[0].name.length > 50 ? 100 : 50,
            borderWidth: 1,
            borderColor: '#DADADA',
            borderRightWidth: 0,
            border: 10,
        ...
        <View style={{width: '50%', backgroundColor: AppTheme.WHITE}}>
          ...
          <View
            style={{
              height:
                studentName[0] && studentName[0].name.length > 50 ? 75 : 25,
              borderWidth: 1,
              borderColor: '#DADADA',
              borderTopWidth: 0,
              border: 10,
            ...
            {!minimalFlag && (
              <Text style={{marginLeft: 10}}>
                {studentName.length > 0 && studentName[0].name}
              </Text>
            )}
          </View>
        </View>
        ...
        <ModalPopup
          ...
          borderCutomStyle={[
            styles.borderStyle,
            {borderColor: themeColor1 ? themeColor1 : AppTheme.GREEN},
          ]}
          data={
            <View style={{}}>
              <Text style={styles.textStyle}>
                ...
                <Text style={{fontWeight: '400'}}>{`${
                  scanitemdata && scanitemdata.studentId
                }`}</Text>
              </Text>
              {!minimalFlag && (
                <Text style={styles.textStyle}>
                ...
                {BrandLabel && MARKS_INFO ? (
                  <View style={{flexDirection: 'row', width: '100%'}}>
                    <MarksHeaderTable
                      ...
                      rowTitle={
                        (BrandLabel && BrandLabel.questionId) ||
                        MARKS_INFO.questionId
                      }
                    ...
                    <MarksHeaderTable
                      ...
                      rowTitle={
                        (BrandLabel && BrandLabel.obtainedMarks) ||
                        MARKS_INFO.obtainedMarks
                      }
                      rowBorderColor={AppTheme.TAB_BORDER}
                      editable={false}
                    />
              ...
              {scanitemdata &&
                scanitemdata.marksInfo.map((M, i) => {
                  return (
